...
module.exports = class Usuario {
  ...
  async create() {
    ...
    const operacaoAssincrona = new Promise((resolve, reject) => {
      ...
      let sql =
        "INSERT INTO `tcc`.`Usuario` (`id`, `p_nome`, `sobrenome`, `username`, `email`, `senha`, `cidade`, `estado`) VALUES (?,?,?,?,?,?,?,?);";
  ...
  async read() {
    const operacaoAssincrona = new Promise((resolve, reject) => {
      ...
      if (email == null) {
        SQL =
          "SELECT id,p_nome,sobrenome,username,email,cidade,estado FROM Usuario ORDER BY email";
      }
      if (email != null) {
        SQL =
          "SELECT id,p_nome,sobrenome,username,email,cidade,estado FROM Usuario where email=? ORDER BY email;";
      }
  ...
  async update() {
    ...
    const operacaoAssincrona = new Promise((resolve, reject) => {
      ...
      const sql =
        "update Usuario set p_nome=?,sobrenome=?,username=?,cidade=?,senha=?,estado=? where id = ?";
      ...
      this._banco.query(sql, parametros, (error, result) => {
        ...
        if (error) {
          ...
          if (result[0].qtd > 0) {
            ...
            const secret = new TextEncoder().encode(process.env.passworld);
            const alg = "HS256";
